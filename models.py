...
def generate_speech(
    ...
    try:
        ...
        for count, (_, ps, audio) in enumerate(generator, start=1):
            if count > max_segments:
                logger.warning("Reached max segment limit during generation")
                break
            ...
            if isinstance(audio, np.ndarray):
                ...
                logger.warning(f"Skipping unsupported audio type: {type(audio)}")
                continue
        ...
        if len(all_audio) == 1:
            ...
            try:
                ...
                logger.error(f"Failed to concatenate audio segments: {exc}")
                return None, None

        return final_audio, phonemes
